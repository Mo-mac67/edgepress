...
import { addReply } from "@/lib/forum-store";
import { clientIp, rateLimit } from "@/lib/rate-limit";
import { isSpam } from "@/lib/spam";

export const dynamic = "force-dynamic";

/** Reply to an approved topic — always lands in the moderation queue. */
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body._hp) return NextResponse.json({ ok: true, pending: true });
  const spam = isSpam(`${body.author ?? ""} ${body.body ?? ""}`);
  const result = await addReply({ threadId: id, body: body.body, author: body.author, spam });
  if ("error" in result) return NextResponse.json(result, { status: result.error === "Unknown topic" ? 404 : 422 });
